...
class LoginLocalUserUseCase(Generic[SessionType]):
    ...
    async def execute(self, session: SessionType, email: str, password: str, client_meta: ClientMetadata | None = None) -> tuple[UserIdentity, str]:
        """
        Authenticate a user. 
        ...
        stored_hash = await self._user_repo.find_password_hash(session, user.id)
