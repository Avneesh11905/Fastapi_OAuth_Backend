...
class ChangePasswordUseCase(Generic[SessionType]):
    ...
    async def execute(self, session: SessionType, user_id: str, current_password: str | None, new_password: str) -> None:
        ...
        stored_hash = await self._user_repo.find_password_hash(session, user_id)
        ...
        await self._user_repo.update_password(session, user_id, new_hash)
